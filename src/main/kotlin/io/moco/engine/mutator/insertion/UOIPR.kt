...
class UOIPR(
                ...
                if (tracker.mutatedClassTracker.targetMutationID != null) {
                    if (tracker.isTargetMutation(newMutation.mutationID)) {
                        ...
                        when (operatorType.second) {
                            "POI" -> visited = handleVarPostOp(opcode, v)
                            "POD" -> visited = handleVarPostOp(opcode, v, false)
                        }
                        if (visited) break
                    }
                }
    ...
    private fun handleVarPostOp(opcode: Int, v: Int, isInc: Boolean = true): Boolean {
        when (opcode) {
            Opcodes.ILOAD -> {
            ...
            for (operatorType in operatorTypes) {
                ...
                if (tracker.mutatedClassTracker.targetMutationID != null) {
                    if (tracker.isTargetMutation(newMutation.mutationID)) {
                        ...
                        when (operatorType.second) {
                            "PRI" -> visited = handleArrPostOp(opcode)
                            "PRD" -> visited = handleArrPostOp(opcode, false)
                        }
                        if (visited) break
                    }
                }
